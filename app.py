...
def verify():
    ...
    try:
        ...
        if result.get("error"):
            ...
            verification_text = {
                "facesMatch": "Yes" if result["verified"] else "No",
                "distance": f"{float(result['distance']):.4f}",
                "similarityScore": f"{float(result['similarity_score']):.4f}",
                "threshold": f"{float(result['threshold']):.4f}",
                "modelUsed": str(result["model"]),
                "detectorBackend": str(result["detector_backend"])
            }
            ...
            response = {
                ...
                "display": {
                    "title": "Face Verification Results:",
                    "details": [
                        f"Faces Match: {verification_text['facesMatch']}",
                        f"Distance: {verification_text['distance']}",
                        f"Similarity Score (cosine based): {verification_text['similarityScore']}",
                        f"Threshold: {verification_text['threshold']}",
                        f"Model Used: {verification_text['modelUsed']}",
                        f"Detector Backend: {verification_text['detectorBackend']}"
                    ]
                }
            }
