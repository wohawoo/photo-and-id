...
app.config['UPLOAD_FOLDER'] = '/dev/shm/face_verification' if os.path.exists('/dev/shm') else tempfile.gettempdir()
...
def optimize_image(image_path):
    ...
    try:
        # Read image in grayscale to use less memory
        ...
        logger.debug(f"Optimized image {image_path} to {img.shape}")
        del img
        gc.collect()
        return True
    except Exception as e:
...
def verify_faces(photo_path, license_path):
    ...
    try:
        ...
        if not optimize_image(photo_path) or not optimize_image(license_path):
            raise ValueError("Failed to optimize images")

        # Import DeepFace only when needed to save memory
